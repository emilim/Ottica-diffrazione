...
def calcola_valori(minimo):
    ...
    s_min_rad = np.sqrt(minimo**2 * s_conv**2 + np.power(0.0191, 2)*9/12)  # s_x = deltapassi/sqrt(12)
    
    return minimo_mrad, s_min_rad

# Caricare il file di testo con i dati
file_path = 'all_minimi_3fenditure.txt'  # Cambia il percorso del file di input
...
output_file_path = 'all_risultati_minimi.txt'  # Cambia il percorso del file di output
